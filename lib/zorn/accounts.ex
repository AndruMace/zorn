defmodule Zorn.Accounts do
  ...
  alias Zorn.Accounts.{Users, UsersToken, UsersNotifier}
  ...
  See `Zorn.Accounts.Users.email_changeset/3` for a list of supported options.
  ...
  See `Zorn.Accounts.Users.password_changeset/3` for a list of supported options.
  ...
  2. The users has not confirmed their email. In this case, the users
     gets confirmed, logged in, and all tokens - including session ones -
     are expired. In theory, no other tokens exist but we delete all of
     them for best security practices.
  """
  ...
  ## Examples
      ...
      iex> deliver_users_update_email_instructions(users, current_email, &url(~p"/user/settings/confirm-email/#{&1}"))
      {:ok, %{to: ..., body: ...}}
