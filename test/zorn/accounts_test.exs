defmodule Zorn.AccountsTest do
  use Zorn.DataCase

  alias Zorn.Accounts

  import Zorn.AccountsFixtures
  alias Zorn.Accounts.{Users, UsersToken}
  ...
  describe "register_users/1" do
    ...
    test "registers user without password" do
      ...
      attrs =
        valid_users_attributes(email: email)
        |> Map.drop([:password, :password_confirmation])

      {:ok, users} = Accounts.register_users(attrs)
  ...
  describe "change_users_email/3" do
    test "returns a users changeset" do
      ...
      assert :username in changeset.required
    end
  end
  ...
  describe "login_users_by_magic_link/1" do
    ...
    test "confirms unconfirmed users even when a password is set" do
      users = unconfirmed_users_fixture(with_password: true)
      assert users.hashed_password
      {encoded_token, hashed_token} = generate_users_magic_link_token(users)

      assert {:ok, {updated_users, [%{token: ^hashed_token}]}} =
               Accounts.login_users_by_magic_link(encoded_token)

      assert updated_users.confirmed_at
    end
  end
