defmodule ZornWeb.UsersAuthTest do
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, ZornWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_users/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      ZornWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "on_mount :require_sudo_mode" do
    test "allows user that have authenticated in the last 10 minutes", %{conn: conn, users: users} do
      users_token = Accounts.generate_users_session_token(users)
      session = conn |> put_session(:users_token, users_token) |> get_session()
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        ZornWeb.Endpoint.subscribe("user_sessions:#{Base.url_encode64(token)}")
      end
